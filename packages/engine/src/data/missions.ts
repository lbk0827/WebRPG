...
export const MISSIONS: Mission[] = [
  {
    ...
    title: '패턴의 순서',
    brief:
      '훈련 교관과 1:1 대련이다. 내 전사는 강타를 알고 있지만, 수칙 1번이 "항상 → 기본 공격"이라 강타 패턴까지 내려가지 못한다. 수칙은 위에서부터 읽고, 처음 맞는 패턴에서 멈춘다.',
    goal: '패턴의 순서만 바꿔서 교관을 이겨라',
    lesson: '수칙은 위에서부터 평가되고, 처음 참인 패턴에서 멈춘다',
    hint: '"항상"은 언제나 참이다. 항상인 패턴 아래의 패턴은 절대 실행되지 않는다. 강타를 위로 올려라.',
    seed: 101,
  ...
  },
  {
    ...
    title: '빈손으로 서 있지 마라',
    brief:
      '마법사의 수칙은 "SP 22 이상 → 대화염" 하나뿐이다. SP가 떨어지면 맞는 패턴이 없어 우물쭈물하며 차례를 넘긴다. 전사가 앞에서 버티는 동안 마법사는 멍하니 서 있다.',
    goal: 'SP가 떨어져도 뭔가 하도록 패턴을 추가해서 이겨라',
    lesson: '모든 패턴이 거짓이면 아무것도 하지 않는다 — 마지막엔 항상 실행되는 패턴(fallback)을 둔다',
  ...
  },
  {
    ...
    title: '치료는 늦으면 소용없다',
    brief:
      '프리스트의 치유 패턴은 "아군 HP 10% 이하 → 치유"다. 전사는 10%가 되기 전에 한 방에 쓰러진다. 임계값이 문제다.',
  ...
  },
  {
    ...
    hint: '도적의 1번 패턴에 "적군 시전 중 1명 이상 → 침묵"을 넣어라. 흔들기도 된다.',
    seed: 105,
    player: [
      { job: 'warrior', guard: { mode: 'always' }, rules: rules(row(atom({ kind: 'selfSpAbs', cmp: 'gte', value: 8 }), 'heavyBlow'), row(always, 'strike')) },
  ...
  },
  {
    ...
    title: '함성은 한 번이면 된다',
    brief:
      '전사의 1번 패턴이 "항상 → 전의 고양"이다. 공격력은 오르지만, 매 차례 함성만 지르다 SP가 바닥난 뒤에야 칼을 든다. 첫 행동에만 쓰고 싶다.',
    goal: '전의 고양을 딱 한 번만 쓰게 만들어 이겨라',
    lesson: '"N회만"과 "내 N번째 행동" — 오프닝 무브를 지정하는 두 방법',
    hint: '패턴 오른쪽의 "회만"에 1을 넣거나, 조건을 "내 행동 횟수 정확히 1"로 바꿔라.',
    seed: 106,
  ...
  },
  {
    ...
    title: '쓰러진 자를 일으켜라',
    brief:
      '적 암살자가 첫 두 발로 엘프를 쓰러뜨린다. 막을 수는 없다. 하지만 프리스트는 소생을 안다 — 소생은 "쓰러진 아군"이 있을 때만 대상을 찾고, 없으면 그 패턴은 건너뛰고 다음으로 간다. 그러니 맨 위에 둬도 안전하다.',
    goal: '쓰러진 엘프를 한 번이라도 일으켜 세우고 이겨라 (이기는 것만으로는 부족하다)',
    lesson: '한정형 대상 스킬(소생·침묵·정화)은 대상이 없으면 실패하고 다음 패턴으로 넘어간다',
    hint: '프리스트 1번 패턴에 "아군 전사자 1명 이상 → 소생"을 넣어라. 전사자가 없을 땐 자동으로 건너뛰니 맨 위가 안전하다.',
    seed: 107,
  ...
  },
  {
    ...
    hint: '1번 패턴에 "적군 생존자 3명 이상 → 휩쓸기"를 넣어라. 셋 이하로 줄면 강타로 넘어간다.',
    seed: 108,
  ...
  },
  {
    ...
    hint: '프리스트 1번 패턴에 "아군 중 [중독] 1명 이상 → 정화"를 넣어라. 정화는 중독자가 없으면 자동으로 건너뛴다.',
    seed: 109,
  ...
  },
  {
    ...
    brief:
      '갑주 파쇄는 적의 방어를 깎지만 위력은 약하다. 전사의 수칙은 SP만 있으면 갑주 파쇄를 반복한다 — 이미 걸려 있는 적에게 또. 강타 패턴은 그 아래에 묻혀 있다. 방어 약화는 한 번이면 충분하고, 그 뒤엔 강타가 제값을 한다.',
    goal: '방어 약화가 걸려 있지 않을 때만 갑주 파쇄를 쓰게 만들어 이겨라',
    lesson: '"N명 정확히 0" 조건 = "없을 때". 상태가 없을 때만 거는 법',
    hint: '갑주 파쇄 패턴의 조건을 "적군 중 [방어 약화] 0명 정확히"로 바꿔라. 걸려 있으면 건너뛰고 강타로 간다.',
    seed: 110,
    player: [
      {
        job: 'warrior',
  ...
  },
  {
    ...
    hint: '프리스트 1번 패턴에 "아군 평균 HP 65% 이하 → 기원"을 넣어라. 치유 패턴은 그 아래에 남겨둔다.',
    seed: 111,
    player: [
  ...
  },
  {
    ...
    hint: '대화염 패턴을 지우거나 맨 아래로 내려라. "내 SP 6 이상 → 마력탄", "항상 → 명상"이면 충분하다.',
    seed: 112,
    player: [
      { job: 'warrior', guard: { mode: 'always' }, rules: strikeOnly },
