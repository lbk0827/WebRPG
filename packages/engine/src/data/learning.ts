...
export function learnableFor(job: string, job2?: string): Learnable[] {
  // 전직 사슬 전체 — 주인공은 길드원의 목록 위에 용사의 목록이 쌓인다 (docs/20)
  return [...COMMON_LEARNABLE, ...(LEARNABLE[job] ?? []), ...advanceChain(job2).flatMap((a) => a.learnable)]
}
...
export function jobSkillPool(job: string, job2?: string): string[] {
  ...
  return [...starter, ...advanceChain(job2).flatMap((a) => a.grants), ...learnableFor(job, job2).map((l) => l.skillId)]
}
