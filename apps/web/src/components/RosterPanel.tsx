// 단원 (M2-1). 편성 5슬롯 + 단원 성장(레벨·경험치·스탯 분배) + 저장 내보내기/가져오기.
...
import { exportGame, importGame, newGame } from '../game/save'
...
export function RosterPanel({ save, onSave }: Props) {
  ...
  return (
    <section className="roster">
          ...
          return (
            <li key={m.id} className="member">
              <header>
                ...
                <div>
                  <div className="name">{m.name} <small>{jobName(m.job)} · Lv {m.level}</small></div>
              ...
              <div className="stats">
                {STAT_KEYS.map((k) => (
                  <span key={k} className="stat">
                    ...
                    <button disabled={m.statPoints <= 0 || s[k] >= STAT_CAP} onClick={() => setMember(allocateStat(m, k))} title={`${STAT_LABEL[k]} +1`}>+</button>
                  </span>
                ))}
              </div>
              <div className="skills">{p.skills.map(skillLabel).join(' · ')}{m.skillPoints > 0 && <small> · 스킬 포인트 {m.skillPoints} (스킬트리는 M2-2)</small>}</div>
            </li>
          )
        })}
      </ul>
      ...
      <details className="saveio">
        ...
        <p className="hint">진행은 이 브라우저에 저장됩니다. 다른 기기로 옮기거나 백업하려면 내보내기 → 가져오기.</p>
